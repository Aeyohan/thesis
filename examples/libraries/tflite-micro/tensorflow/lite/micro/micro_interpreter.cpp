  ...
  for (int subgraph_idx = 0; subgraph_idx < graph_.NumSubgraphs();
    ...
    for (size_t i = 0; i < operators_size; ++i) {
      ...
      if (index >= opcodes->size()) {
        NRF_LOG_ERROR("Missing registration for opcode_index %d\n", index);
        return kTfLiteError;
      }
      ...
      TfLiteStatus status =
          GetRegistrationFromOpCode(opcode, op_resolver_, error_reporter_,
                                    &(graph_.GetAllocations()[subgraph_idx]
                                          ...
                                          .registration));
      if (status != kTfLiteOk) {
        NRF_LOG_ERROR("Failed to get registration from op code %s\n ",
      ...
      if (registration == nullptr) {
        NRF_LOG_ERROR("Skipping op for opcode_index %d\n", index);
        return kTfLiteError;
      }
      ...
      if (op_type == BuiltinOperator_CUSTOM) {
        ...
        if (parser == nullptr) {
          NRF_LOG_ERROR("Did not find a parser for %s",
                      EnumNameBuiltinOperator(op_type));
...
TfLiteStatus MicroInterpreter::Invoke() {
  if (initialization_status_ != kTfLiteOk) {
    NRF_LOG_ERROR("Invoke() called after initialization failed\n");
    return kTfLiteError;
  }
  ...
  if (!tensors_allocated_) {
    NRF_LOG_ERROR("Allocating tensors");
    TF_LITE_ENSURE_OK(&context_, AllocateTensors());
    NRF_LOG_ERROR("Allocated tensors");
  }
